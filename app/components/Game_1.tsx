...
interface Cell {
    ...
    message?: string; // Description for green cells
...
const SPECIAL_CELLS: Record<number, Omit<Cell, 'id'>> = {
  ...
  4: {
    ...
    message: "frog consumed termites! Promoted to 6",
    moveBonus: 6
  },
...
const FrogPiece: React.FC<{ color: string; i: number }> = ({ color,i }) => (
    //   <svg
    //     width={size}
    //     height={size}
    //     viewBox="0 0 24 24"
...
const AnimatedDice: React.FC<{ value: number; isRolling: boolean }> = ({ value, isRolling }) => {
    ...
    return (
        <div
            className={`w-16 h-16 bg-white border-4 border-gray-800 rounded-lg grid grid-cols-3 grid-rows-3 gap-1 p-2 transition-transform ${isRolling ? 'animate-spin' : ''
                }`}
        >
            {[...Array(9)].map((_, i) => (
...
export default function FrogRaceGame() {
    ...
    const [modalContent, setModalContent] = useState({ title: '', message: '', effect:'' });
    ...
    if (!gameStarted) {
        return (
            <div className="min-h-screen bg-[url(/bg-forest.jpg)] bg-cover p-4 flex items-center justify-center">
                <div className=" rounded-2xl shadow-2xl p-8 max-w-md w-full border-4 bg-orange-50 border-orange-100">
                    <h1 className="text-4xl font-bold text-center mb-6 text-orange-300">
                        🐸 Hoppy Frog 🐸
                    </h1>
                    ...
                    <div className={` w-full h-full flex flex-col gap-4 p-2`}>
                        {Array.from({ length: numPlayers }, (_, i) => (
                            <input key={i} className="w-full p-4 border-2 border-orange-200 "
        ...
        <div className="h-screen bg-gradient-to-br from-green-100 to-blue-100 p-2 sm:p-3 lg:p-4 overflow-hidden">
            <div className="max-w-[1920px] mx-auto h-full flex flex-col">
                                ...
                                {[...Array(GRID_SIZE)].map((_, rowIdx) => {
                                    ...
                                    return [...Array(GRID_SIZE)].map((_, colIdx) => {
                                        ...
                                        return (
                                            <div
                                                key={`${rowIdx}-${colIdx}`}
                                                className={`aspect-square rounded-lg border-2 flex flex-col items-center justify-center relative transition-all ${cell.type === 'green'
                                                ...
                                                <div className="flex flex-wrap gap-0.5 items-center justify-center">
                                                    {playersOnCell.map((player) => (
                                                        <div key={player.id} title={player.name}>
                                                            <FrogPiece color={player.color} i={player.id} />
                                                        </div>
                                                    ))}
                                                </div>
                    ...
                    <div className="flex flex-col gap-2 min-h-0 overflow-hidden">
                        ...
                        <div className="bg-white rounded-xl shadow-lg p-2 sm:p-3 flex-shrink-0">
                            ...
                            <div className="flex items-center gap-2">
                                <FrogPiece color={players[currentPlayerIndex].color} i={currentPlayerIndex} />
                                <div>
                        ...
                        <div className="bg-white rounded-xl shadow-lg p-2 sm:p-3 flex-shrink-0">
                            ...
                            <div className="space-y-1.5">
                                {players.map((player, idx) => (
                                    <div
                                        key={idx}
                                        className={`flex items-center gap-2 p-1.5 rounded-lg ${idx === currentPlayerIndex ? 'bg-yellow-100 ring-2 ring-yellow-400' : 'bg-gray-50'
                                            }`}
                                    >
                                        <FrogPiece color={player.color} i={player.id} />
                                        <div className="flex-1 min-w-0">
                                            <p className="font-semibold text-[10px] sm:text-xs truncate">{playernames[idx]}</p>
                ...
                {showModal && (
                    ...
                    <div  className="fixed inset-0 bg-black opacity-55  flex items-center justify-center z-5 p-4 text-white"></div>
                    <div className='fixed inset-0    flex items-center justify-center z-5 p-4 text-white '>
                        <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-md w-full ">

                          <h3 id="modal-title" className="text-2xl font-bold mb-4 text-center">
                               {modalContent.title}
                         </h3>
                          <p className="text-xl text-gray-900 text-center border-2">{modalContent.effect}</p>
                            <p className="text-lg text-gray-700 text-center border-2">{modalContent.message}</p>
                    ...
                    >
                        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full text-center">
                            ...
                            <div className="flex justify-center mb-4">
                                <FrogPiece color={winner.color} i={winner.id} />
                            </div>
